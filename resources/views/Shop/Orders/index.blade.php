    ...
    <div class="scrollspy-example" data-spy="scroll" data-target="#account-settings-scroll" data-offset="-100">
        <div class="row">
            ...
            <div class="col-lg-12 col-12  layout-spacing">
                <form id="searchCompanies" name="searchSs" action="{{url('userOrders')}}">
                            ...
                            <div class="row">
                                ...
                                <div class="col-xl-2">
                                    <div class="form-group">
                                        ...
                                        <?php }else{ ?>
                                            <input type="text" readonly="" value="{{$currentShop->name}}" class="form-control form-control-lg">
                                        <?php } ?>
                                    </div>
                                </div>
                                ...
                                <div class="col-xl-2">
                                    <div class="form-group">
                                        <select class="form-control form-control-lg" name="status" id="">
                                            ...
                                            <?php $statusArr = array('pending'=>'pending','payment'=>'payment','approved'=>'approved','in_process'=>'in_process','cancel'=>'cancel','shipped'=>'shipped','complete'=>'complete'); 
                                                    ...
                                                    @foreach ($orders as $row)
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td class="text-center">
                                                                <a href="{{url('/userOrders/'.$row->id.'/')}}" title="View">
                                                                    <div class="icon-container">
...
<script>
    function approveAll(){
        ...
        $.ajax({url: '{{url('/userOrders/approveAll')}}',type:'post',data:data, success: function(result){
            if(result == 'success'){
                $('.checkbox:checked').each(function() {
                    id = this.value;
    ...
    function disapproveAll(){
        ...
        var data = {'_token':'<?php echo csrf_token()?>','ids':ids}
        $.ajax({url: '{{url('/userOrders/cancelAll')}}',type:'post',data:data, success: function(result){
            if(result == 'success'){
                $('.checkbox:checked').each(function() {
                    id = this.value;
    ...
    function approve(elem,id){
        $('.approve_'+id).hide();
        $('.approve_wait_'+id).show();
        $.ajax({url: '{{url('/userOrders/approve')}}/'+id,type:'post',data:'_token=<?php echo csrf_token()?>', success: function(result){
            if(result == 'success'){
                $('.approve_wait_'+id).hide();
                $('.approve_done_'+id).show();
                $('.status_'+id).text('Payment');
    ...
    function cancelOR(elem,id){
        $('.cancel_'+id).hide();
        $('.cancel_wait_'+id).show();
        $.ajax({url: '{{url('/userOrders/cancel')}}/'+id,type:'post',data:'_token=<?php echo csrf_token()?>', success: function(result){
                if(result == 'success'){
                $('.cancel_wait_'+id).hide();
                $('.cancel_done_'+id).show();
                $('.status_'+id).text('Canceled');
