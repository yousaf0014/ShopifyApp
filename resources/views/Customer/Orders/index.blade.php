                    ...
                    <div class="statbox widget box box-shadow">
                        ...
                        <div class="widget-content widget-content-area">
                            <div class="row">
                                ...
                                <div class="col-xl-3">
                                    <div class="form-group">
                                        <select class="form-control form-control-lg" name="store">
                                ...
                                <div class="col-xl-2">
                                    <a href="{{url('storeOrders/file')}}" class="btn btn-success btn-sm">
                                                    ...
                                                    @foreach ($orders as $row)
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td class="text-center">
                                                                <a href="{{url('/storeOrders/'.$row->id.'/')}}" title="View">
                                                                    <div class="icon-container">
...
<script>
    function approveAll(){
        ...
        $.ajax({url: '{{url('/storeOrders/approveAll')}}',type:'post',data:data, success: function(result){
            if(result == 'success'){
                $('.checkbox:checked').each(function() {
                    id = this.value;
    ...
    function disapproveAll(){
        ...
        var data = {'_token':'<?php echo csrf_token()?>','ids':ids}
        $.ajax({url: '{{url('/storeOrders/cancelAll')}}',type:'post',data:data, success: function(result){
            if(result == 'success'){
                $('.checkbox:checked').each(function() {
                    id = this.value;
    ...
    function approve(elem,id){
        $('.approve_'+id).hide();
        $('.approve_wait_'+id).show();
        $.ajax({url: '{{url('/storeOrders/approve')}}/'+id,type:'post',data:'_token=<?php echo csrf_token()?>', success: function(result){
            if(result == 'success'){
                $('.approve_wait_'+id).hide();
                $('.approve_done_'+id).show();
                $('.status_'+id).text('Payment');
    ...
    function cancelOR(elem,id){
        $('.cancel_'+id).hide();
        $('.cancel_wait_'+id).show();
        $.ajax({url: '{{url('/storeOrders/cancel')}}/'+id,type:'post',data:'_token=<?php echo csrf_token()?>', success: function(result){
                if(result == 'success'){
                $('.cancel_wait_'+id).hide();
                $('.cancel_done_'+id).show();
                $('.status_'+id).text('Canceled');
