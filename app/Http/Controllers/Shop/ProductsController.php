...
class ProductsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $productObj = $productObj->where('user_id',Auth::user()->id);
        if(!empty($request->keyword)){
            $keyword = $request->keyword;
            $productObj = $productObj->where('title','like', '%'.$keyword.'%');
        }        
        $products = $productObj->paginate(20);
        return view('Shop.Products.index',compact('products','keyword'));
    }
    ...
    public function create(Request $request, $category = null){
        ...
        return View('Shop.Products.create',compact('adminProducts','categories'));
    }
    ...
    public function createProduct(Request $request, AdminProduct $adminProduct){
        ...
        return View('Shop.Products.createProduct',compact('adminProduct','productAttributeGroups'));
    }
    ...
    public function createProductStep2(Request $request, AdminProduct $adminProduct){
        ...
        return View('Shop.Products.createProduct2',compact('colorData','adminProduct','productPrintingGroups'));
    }
    ...
    //private function __addProdcutToShopify(){
    public function addProdcutToShopify(){
        ...
        foreach($userProdcuts as $up){
            ...
            //$webhooks = $shop->api()->rest('Post', '/admin/api/2020-10/webhooks.json');
            $sproduct = $shop->api()->rest('Post', '/admin/api/2020-10/products.json',$product);
            ...
            foreach($up->productPrintingGroupOption as $pics){
                /*if(!empty($pics->artwork)){
                    ...
                    $images[] = $shop->api()->rest('Post',"/admin/products/".$productID.'/images.json',$productImages);
                }*/
                if(!empty($pics->shirt_design)){
                    $productImages = array('image'=>array('src'=>$path.$pics->shirt_design));
                    $image = $shop->api()->rest('Post',"/admin/api/2020-10/products/".$productID.'/images.json',$productImages);
            ...
            if(!empty($productVarients) && !empty($images)){
                foreach($productVarients as $varnt){
                    foreach($images as $img){
                        ...
                        $response = $shop->api()->rest('Put','/admin/api/2020-10/variants/'.$varnt.'.json',$temp);
                    }
                }
            }
